...
ACTOR_LR=1e-4
CRITIC_LR=1e-4
CONFIG_ACTOR_NET=fcnet-64-64-elu.json
CONFIG_CRITIC_NET=fcnet-64-64-elu.json
...
EXPERIMENT=nav0-td3-baseline-fcnet-64-64
...
tensorboard --logdir=$TEST_RUN/$EXPERIMENT &
...
mapo --run MAPO --env $ENV --model-loss mle --config-actor-net $CONFIG_ACTOR_NET --config-critic-net $CONFIG_CRITIC_NET --config-dynamics-net dynamics-linear-relu.json --actor-lr $ACTOR_LR --critic-lr $CRITIC_LR --dynamics-lr 1e-4 --train-batch-size $TRAIN_BATCH_SIZE --num-samples $NUM_SAMPLES --timesteps-total $TIMESTEPS_TOTAL --name $TEST_RUN/$EXPERIMENT --num-cpus-for-driver $NUM_CPUS_FOR_DRIVER --num-gpus $NUM_GPUS --debug
...
mapo --run MAPO --env $ENV --model-loss mle --config-actor-net $CONFIG_ACTOR_NET --config-critic-net $CONFIG_CRITIC_NET --config-dynamics-net fcnet-64-elu.json --actor-lr $ACTOR_LR --critic-lr $CRITIC_LR --dynamics-lr 1e-4 --train-batch-size $TRAIN_BATCH_SIZE --num-samples $NUM_SAMPLES --timesteps-total $TIMESTEPS_TOTAL --name $TEST_RUN/$EXPERIMENT --num-cpus-for-driver $NUM_CPUS_FOR_DRIVER --num-gpus $NUM_GPUS --debug

tensorboard --logdir=$TEST_RUN/$EXPERIMENT &


EXPERIMENT=nav0-mapo-pga-fcnet-64-64
mapo --run MAPO --env $ENV --model-loss pga --config-actor-net $CONFIG_ACTOR_NET --config-critic-net $CONFIG_CRITIC_NET --config-dynamics-net fcnet-64-elu.json --actor-lr $ACTOR_LR --critic-lr $CRITIC_LR --dynamics-lr 1e-4 --train-batch-size $TRAIN_BATCH_SIZE --num-samples $NUM_SAMPLES --timesteps-total $TIMESTEPS_TOTAL --name $TEST_RUN/$EXPERIMENT --num-cpus-for-driver $NUM_CPUS_FOR_DRIVER --num-gpus $NUM_GPUS --debug
